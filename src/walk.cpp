...
void Solver::walkstep()
{
	...
	if (tracker.best < UNDEF_VAR) {
		const uint32 limit = (inf.maxVar >> 2) + 1;
		assert(limit < UNDEF_VAR);
	...
	if (tracker.current < tracker.minimum) {
		...
		if (tracker.best == UNDEF_VAR)
			...
			assert(tracker.trail.size() < UNDEF_VAR);
...
void Solver::walkinit()
{
	...
	assert(!bot.empty());
	assert(ORIGINALS < UINT32_MAX);
	const uint32 nclauses = (uint32)ORIGINALS;
	tracker.nclauses = nclauses;
	tracker.orgs.resize(nclauses);
	tracker.cinfo.resize(nclauses);
	tracker.value = sfmalloc<LIT_ST>(inf.nDualVars);
	assert(tracker.value != NULL);
	memset(tracker.value, UNDEF_VAL, inf.nDualVars);
}

bool Solver::walkschedule()
{
	assert(tracker.nclauses == ORIGINALS);
...
void Solver::walkstop()
{
	...
	if (tracker.minimum == tracker.initial) {
		...
		if (tracker.best && NEQUAL(tracker.best, UNDEF_VAR))
...
inline uint32 Solver::breakValue(const uint32& lit)
{
	...
	forall_bol(list, i) {
		...
		const CINFO& cinfo = tracker.cinfo[*i];
		if (cinfo.size == 1) 
			breaks++;
	}
	return breaks;
